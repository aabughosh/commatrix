...
The `kubectl commatrix` plugin uses the `commatrix` command, including the `generate` command, which generates an up-to-date communication flows matrix for all ingress flows in Kubernetes (both multi-node and single-node in OpenShift) and Operators.
...
$ kubectl commatrix generate --host-open-ports
```
